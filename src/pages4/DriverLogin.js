...
const DriverLoginForm = () => {
  ...
  const handleIdentifierChange = (e) => {
    ...
    // If user is typing only numbers (i.e., trying to input phone number)
    if (isOnlyDigits(value)) {
      if (value.length <= 10) {
        setIdentifier(value);
      }
    } else {
      // Allow all valid email characters
  ...
  const handleLogin = async (e) => {
    ...
    const isEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(identifier);
    const isPhone = /^\d{10}$/.test(identifier); // Strict 10-digit check
